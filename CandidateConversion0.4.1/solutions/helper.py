...
def find_file_name(name, fileNames):
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p","--path", help = "path to all files", required = True)
    parser.add_argument("-r","--resumevalues", help = "resume values path", required = True)
    args = parser.parse_args()
    path = args.path
    resumevaluespath = args.resumevalues
    if not os.path.exists(path):
        raise Exception("Path not valid")
    if not os.path.exists(resumevaluespath):
        raise Exception("Resume values path is not valid")
        
    fileNames = retrieve_file_paths_zip(path)
    if not os.path.exists(path + "\\AllBatches"):
        os.mkdir(path + "\\AllBatches")
        os.mkdir(path + "\\AllBatches\\BlobFiles")

    print("Decompressing and gathering all batch files")
    for file in fileNames:
        nonzipFile = file.replace(".zip","")
        decompress(file)
        #os.remove(file)
        blobFileNames = retrieve_file_paths_nonTuple(nonzipFile + "\\BlobFiles")
        for blobFile in blobFileNames:
            shutil.copy(blobFile, path  + "\\AllBatches\\BlobFiles")
        shutil.rmtree(nonzipFile)
    path = path + "\\AllBatches"
    
    print("Writing Candidate.xlsx")
    resumevalues = pd.read_csv(resumevaluespath, dtype = str)
    for column in resumevalues.columns:
        if "Unnamed" in column:
            resumevalues = resumevalues.drop(column,axis = 1)
    resumevalues = resumevalues.drop(["FileSize"], axis = 1)
    resumevalues["FileName"] = None
    fileNames = retrieve_file_paths_nonTuple(path+"\\BlobFiles")
    for i in resumevalues.index:
        posname = resumevalues["CandidateID"][i] + "_" + resumevalues["DocNum"][i]
        #print(posname)
        name = find_file_name(posname, fileNames)
        if name != -1:
            resumevalues["FileName"][i] = name
    resumevalues.to_excel(path + "\\Candidate.xlsx", index = False)
